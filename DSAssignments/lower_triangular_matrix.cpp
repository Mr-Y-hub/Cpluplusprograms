...
//
// access element --->   M[i,j] = array[(i*(i-1)/2) + (j-1)]  --> row major
// 
// access element --->   M[i,j] = array[( (n*(j-1)) - (j-2)(j-1)/2 )  +  (i-j) ]  --> column major
...
int main()
{
    Lower_Triangular_Matrix obj(5);
    cout<<"Enter Number to show in Lower triangular Matrix : ";
    int num;
    cin>>num;
    for (int i = 1; i <= 5; i++)
    {
